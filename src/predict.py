...
class Predictor:
    def __init__(self, config: dict):
        ...
        self.metadata = json.loads((artifact_dir / config["artifacts"]["metadata_file"]).read_text(encoding="utf-8"))
    ...
    def predict(self, observations: pd.DataFrame) -> dict:
        ...
        value = max(0.0, float(self.model.predict(latest.reindex(columns=columns))[0]))
        thresholds = self.config["thresholds"]
        tree_predictions = []
        try:
            transformed = self.model.named_steps["preprocess"].transform(latest.reindex(columns=columns))
            tree_predictions = [tree.predict(transformed)[0] for tree in self.model.named_steps["model"].estimators_]
        except (AttributeError, KeyError):
            pass
        uncertainty = float(np.std(tree_predictions)) if tree_predictions else None
        confidence = None if uncertainty is None else float(np.clip(1 - uncertainty / max(value, 1), 0, 1))
        return {
